                                        ...
                                        @foreach ($product->product_prices->where('jenis', 1) as $stock)
                                            @if ($stock->jumlah > 0)
                                                <li>
                                                    ...
                                                    <input id="checkbox-item-{{ $stock->ukuran }}-male-{{ $stock->warna }}" type="checkbox" value="{{ $stock->id }}" data-ukuran="{{ $stock->ukuran }}" data-warna="{{ $stock->color->warna }}" data-harga-produk="@if($product->status == 'promo') <s>@rupiah($stock->harga)</s> @rupiah($stock->harga - $stock->harga*($stock->diskon/100)) (-{{$stock->diskon}}%)  @else @rupiah($stock->harga) @endif" data-jenis="{{$stock->jenis}}" data-stok-produk="{{ $product->status === 'preorder' ? 2500 : $stock->jumlah }}" class="MaleAdult-checkbox w-4 h-4 text-green-600 bg-gray-100 rounded border-gray-300 focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                                    <label for="checkbox-item-{{ $stock->ukuran }}-male-{{ $stock->warna }}" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $stock->ukuran }} | Warna : {{ Str::ucfirst($stock->color->warna) }} | {{ $product->status === 'preorder' ? 'Preorder' : ($stock->jumlah < 100 ? $stock->jumlah : '+99') }} item | @if($product->status == 'promo') <s>@rupiah($stock->harga)</s> @rupiah($stock->harga - $stock->harga*($stock->diskon/100)) (-{{$stock->diskon}}%)  @else @rupiah($stock->harga) @endif</label>
                                                    </div>
                                                </li>
                                            @endif
                                        @endforeach
                                    ...
                                    <ul class="list-none ml-0 p-3 space-y-3 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMaleKid">
                                        @foreach ($product->product_prices->where('jenis', 3) as $stock)
                                            @if ($stock->jumlah > 0)
                                                <li>
                                                    <div class="flex items-center">
                                                    <input id="checkbox-item-{{ $stock->ukuran }}-male-{{ $stock->warna }}" type="checkbox" value="{{ $stock->id }}" data-ukuran="{{ $stock->ukuran }}" data-warna="{{ $stock->color->warna }}" data-harga-produk="@if($product->status == 'promo') <s>@rupiah($stock->harga)</s> @rupiah($stock->harga - $stock->harga*($stock->diskon/100)) (-{{$stock->diskon}}%)  @else @rupiah($stock->harga) @endif" data-jenis="{{$stock->jenis}}" data-stok-produk="{{ $product->status === 'preorder' ? 2500 : $stock->jumlah }}" class="MaleKid-checkbox w-4 h-4 text-green-600 bg-gray-100 rounded border-gray-300 focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                                    <label for="checkbox-item-{{ $stock->ukuran }}-male-{{ $stock->warna }}" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $stock->ukuran }} | Warna : {{ Str::ucfirst($stock->color->warna) }} | {{ $product->status === 'preorder' ? 'Preorder' : ($stock->jumlah < 100 ? $stock->jumlah : '+99') }} item | @if($product->status == 'promo') <s>@rupiah($stock->harga)</s> @rupiah($stock->harga - $stock->harga*($stock->diskon/100)) (-{{$stock->diskon}}%)  @else @rupiah($stock->harga) @endif</label>
                                                    </div>
                                                </li>
                                            @endif
                                    ...
                                    <ul class="list-none ml-0 p-3 space-y-3 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownFemaleAdult">
                                        @foreach ($product->product_prices->where('jenis', 0) as $stock)
                                            @if ($stock->jumlah > 0)
                                                <li>
                                                    <div class="flex items-center">
                                                    <input id="checkbox-item-{{ $stock->ukuran }}-female-{{ $stock->warna }}" type="checkbox" value="{{ $stock->id }}" data-ukuran="{{ $stock->ukuran }}" data-warna="{{ $stock->color->warna }}" data-harga-produk="@if($product->status == 'promo') <s>@rupiah($stock->harga)</s> @rupiah($stock->harga - $stock->harga*($stock->diskon/100)) (-{{$stock->diskon}}%)  @else @rupiah($stock->harga) @endif" data-jenis="{{$stock->jenis}}" data-stok-produk="{{ $product->status === 'preorder' ? 2500 : $stock->jumlah }}" class="FemaleAdult-checkbox w-4 h-4 text-green-600 bg-gray-100 rounded border-gray-300 focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                                    <label for="checkbox-item-{{ $stock->ukuran }}-female-{{ $stock->warna }}" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $stock->ukuran }} | Warna : {{  Str::ucfirst($stock->color->warna) }} | {{ $product->status === 'preorder' ? 'Preorder' : ($stock->jumlah < 100 ? $stock->jumlah : '+99') }} item | @if($product->status == 'promo') <s>@rupiah($stock->harga)</s> @rupiah($stock->harga - $stock->harga*($stock->diskon/100)) (-{{$stock->diskon}}%)  @else @rupiah($stock->harga) @endif</label>
                                                    </div>
                                                </li>
                                            @endif
                                        @endforeach
                                    ...
                                    <ul class="list-none ml-0 p-3 space-y-3 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownFemaleKid">
                                        @foreach ($product->product_prices->where('jenis', 2) as $stock)
                                            @if ($stock->jumlah > 0)
                                                <li>
                                                    <div class="flex items-center">
                                                    <input id="checkbox-item-{{ $stock->ukuran }}-male-{{ $stock->warna }}" type="checkbox" value="{{ $stock->id }}" data-ukuran="{{ $stock->ukuran }}" data-warna="{{ $stock->color->warna }}" data-harga-produk="@if($product->status == 'promo') <s>@rupiah($stock->harga)</s> @rupiah($stock->harga - $stock->harga*($stock->diskon/100)) (-{{$stock->diskon}}%)  @else @rupiah($stock->harga) @endif" data-jenis="{{$stock->jenis}}" data-stok-produk="{{ $product->status === 'preorder' ? 2500 : $stock->jumlah }}" class="FemaleKid-checkbox w-4 h-4 text-green-600 bg-gray-100 rounded border-gray-300 focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                                    <label for="checkbox-item-{{ $stock->ukuran }}-male-{{ $stock->warna }}" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $stock->ukuran }} | Warna : {{ Str::ucfirst($stock->color->warna) }} | {{ $product->status === 'preorder' ? 'Preorder' : ($stock->jumlah < 100 ? $stock->jumlah : '+99') }} item | @if($product->status == 'promo') <s>@rupiah($stock->harga)</s> @rupiah($stock->harga - $stock->harga*($stock->diskon/100)) (-{{$stock->diskon}}%)  @else @rupiah($stock->harga) @endif</label>
                                                    </div>
                                                </li>
                                            @endif
